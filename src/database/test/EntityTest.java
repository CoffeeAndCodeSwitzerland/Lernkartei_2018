...
class EntityTest {
	public static void myTest() throws Exception {
		...
		assertEquals("PK_TESTTAB INTEGER PRIMARY KEY AUTOINCREMENT,KEY_NAME TEXT,KEY_VALUE TEXT", ea);
		...
		assertEquals("PK_TESTTAB, KEY_NAME, KEY_VALUE", k);

		String k0 = e.getMyAttributes().toKeyList(true);
		debug.Debugger.out("Attr.: {"+k0+"}");
		assertEquals("PK_TESTTAB, KEY_NAME, KEY_VALUE", k0);

		e.addAttributes(atts);
		AttributeList eaL2 = e.getMyAttributes();
		assertEquals(5, eaL2.getSize());
		
		ResultType result = e.createTableIfNotExists();
		assertEquals(ResultType.DONE, result);
		...
		assertEquals("CREATE TABLE IF NOT EXISTS TESTTAB (PK_TESTTAB INTEGER PRIMARY KEY AUTOINCREMENT,KEY_NAME TEXT,KEY_VALUE TEXT,Name TEXT,Age INTEGER)", d);
		ResultType r = e.createTableIfNotExists();

		String last = e.myDBDriver.getLastSQLCommand();
		debug.Debugger.out("SQL("+r+"): {"+last+"}");
		assertEquals(ResultType.DONE, r);
